...
labels = struct( ...
    ...
    'sa_lnn_unet_cl', 'SA-LIB-U-Net-CL', ...
    'ma_lnn_unet_cl', 'MA-LIB-U-Net-CL');
...
plot_train_val_curve(TpropF.epoch, TpropF.train_total, TpropF.val_total, ...
    'Forward Loss Curve of MA-LIB-U-Net-CL', ...
    'Loss', fullfile(outDir, 'proposed_forward_loss_curve'));

TpropI = read_loss_table(experiments.ma_lnn_unet_cl.inverse);
plot_train_val_curve(TpropI.epoch, TpropI.train_inv_raw, TpropI.val_inv_raw, ...
    'Training and Validation Inversion Loss of MA-LIB-U-Net-CL', ...
    'Inversion Loss', fullfile(outDir, 'proposed_inverse_loss_curve'));
...
function add_forward_inset(fig, curves)
    ...
    for i = 1:size(curves, 1)
        ...
        plot(axInset, curve.epoch, curve.loss, lineStyle, ...
            ...
            'Color', color);
        if strcmp(label, 'MA-LIB-U-Net-CL')
